...
export default function DenseTable() {
  ...
  const sendPostRequest = async (id) => {
    ...
    try {
      ...
      setUpdateMessage(`Start processing file ID ${id}... `);
      const resp = await axios.post(INDEX_FILE_PROCESS_URL, requestData, safeHeaders);
      //console.log('response data', resp.data);
      const message = resp.data.message + ` ${resp.data.rowNumber} rows inserted out of ${resp.data.fileName}`;
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setIsLoading(true);
        ...
        try {
        const result = await axios.get(INDEX_DATA_FILE_URL, safeHeaders);
